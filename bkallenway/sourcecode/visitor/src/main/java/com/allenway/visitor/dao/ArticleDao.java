...
public interface ArticleDao extends JpaRepository<Article, String> {
    ...
    @Query("select article.id,article.title,article.tagId from Article article where moduleId=:moduleId and isDelete=:isDelete order by title")
     ...
     */
    @Query("select article.id,article.title,article.tagId,article.isDelete from Article article where tagId=:tagId order by title")
