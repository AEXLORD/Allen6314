...
router.get('/get-articles-by-classify',function(req,res,next){

    logger.debug("visitor/article.js -- /article/get-articles-by-classify ...");
    logger.debug("classify id = " + req.query.id);
    ...
    async.waterfall([
            ...
            //请求 主页文章 数据
                ...
                request(url = config.getBackendUrlPrefix() + "article/get-all-articles-by-classify/" + req.query.id,function(error,response,body){
                    if(!error && response.statusCode == 200){
                        var returnData = JSON.parse(body);
                        if(returnData.statusCode != 0){
                            logger.error("visitor/article.js -- /article/get-articles-by-classify fail ..." +
                                "response.statusCode = 200, but returnData.statusCode = " + returnData.statusCode);
                            res.render('error/unknowerror');
                        } else {
